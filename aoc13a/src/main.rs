...
fn get_intersect(note: &Vec<String>) -> Vec<usize> {
    // Initialize intersection to be a big range to start
        ...
        for j in 0..mod_note.len() {
            ...
            if j % 2 == 1 {
                ...
                for k in 0..=j.min(mod_note.len() - j - 1) {
                    ...
                    let forward_vec: Vec<char> =
                        mod_note[j - k..=j + k].chars().collect::<Vec<char>>();
                    let reverse_vec: Vec<char> =
                        mod_note[j - k..=j + k].chars().rev().collect::<Vec<char>>();
...
fn main() {
    ...
    for note in &mut notes_vec {
        ...
        // Get mirror index for the transposed note, i.e. split within column
        let vert_index_vec: Vec<usize> = get_intersect(&transposed);

        // Initialize hori_index to a high value to determine when it is not found
        let mut vert_index_og: usize = 100000;

        // Make sure there is an element in the returned value
        if vert_index_vec.len() == 1 {
            vert_index_og = (vert_index_vec[0] + 1) / 2;
        }

        // Fix smudge by going through all points one by one
        let mut done: usize = 0;
        for i in 0..note.len() {
            // Exit outer loop if inner loop is done
            if done == 1 {
                break;
            }
            for j in 0..note[i].len() {
                // Obtain char a note[i][j] and invert it
                let char_ij: char = note[i].chars().collect::<Vec<char>>()[j];
                let inverse_char: char = if char_ij == '#' { '.' } else { '#' };

                // Create a copy of the note and replace the char by its inverse char
                let mut mod_note: Vec<String> = note.clone();
                mod_note[i].replace_range(j..=j, &inverse_char.to_string());

                // Get mirror index for the original note, i.e. split within row
                let hori_index_vec: Vec<usize> = get_intersect(&mod_note);

                // Initialize hori_index to a high value to determine when it is not found
                let mut hori_index: usize = 100000;

                // Make sure there is an element in the returned value that is not the og reflection
                if hori_index_vec.len() > 0 {
                    if hori_index_vec.len() != 1 || hori_index_vec[0] != 2 * hori_index_og - 1 {
                        let double_hori_index: usize = *hori_index_vec
                            .iter()
                            .filter(|&&e| e != 2 * hori_index_og - 1)
                            .next()
                            .expect("Value not found");
                        hori_index = (double_hori_index + 1) / 2;
                    }
                }

                // Transpose vector of strings to look at split within column
                let mut transposed: Vec<String> = Vec::new();
                for i in 0..mod_note[0].len() {
                    let column: String = mod_note
                        .iter()
                        .map(|line| line.chars().nth(i).unwrap())
                        .collect();
                    transposed.push(column);
                ...
                // Make sure there is an element in the returned value that is not the og reflection
                if vert_index_vec.len() > 0 {
                    if vert_index_vec.len() != 1 || vert_index_vec[0] != 2 * vert_index_og - 1 {
                        let double_vert_index: usize = *vert_index_vec
                            .iter()
                            .filter(|&&e| e != 2 * vert_index_og - 1)
                            .next()
                            .expect("Value not found");
                        vert_index = (double_vert_index + 1) / 2;
                    }
                }

                // Figure out where the new mirror was found
                if hori_index != 100000 || vert_index != 100000 {
                    if hori_index != 100000 {
                        sum += hori_index;
                    }
                    if vert_index != 100000 {
                        sum += vert_index * 100;
                    }
                    done = 1;
                    break;
